(function($) {
	...
	//保存图片上传至服务器后，服务器返回的文件url
	var photoUrlArray = new Array();
	
    //保存图片url的对象
    function photoUrl(fileName, url) {
    ...
    $(function() {
        ...
        uploader = WebUploader.create({
            ...
            dnd : '#dndArea',
            paste : '#uploader',
            swf : '/css/plugins/webuploader/Uploader.swf',
            chunked : true,
            chunkSize : 512 * 1024,
        ...
        {
        	photoUrlArray.push(new photoUrl(response.fileName, response.url));
        });
        ...
        {
        	var photoUrls = "";
            for (var j = 0; j < photoUrlArray.length; j++) 
            {
            	photoUrls = photoUrls + photoUrlArray[j].fileName + ':' + photoUrlArray[j].url;
            	
            	if (j != (photoUrlArray.length-1))
        		{
            		photoUrls = photoUrls + ",";
        		}
            }
        	
			parent.loadFileNames(photoUrls); //回写至父窗口
